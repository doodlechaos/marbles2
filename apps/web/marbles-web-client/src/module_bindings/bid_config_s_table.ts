...
/**
 * Table handle for the table `BidConfigS`.
 *
 * Obtain a handle from the [`bidConfigS`] property on [`RemoteTables`],
 * like `ctx.db.bidConfigS`.
 *
 * Users are encouraged not to explicitly reference this type,
 * but to directly chain method calls,
 * like `ctx.db.bidConfigS.on_insert(...)`.
 */
export class BidConfigSTableHandle<TableName extends string> implements __TableHandle<TableName> {
  // phantom type to track the table name
  readonly tableName!: TableName;
  tableCache: __TableCache<BidConfigS>;

  constructor(tableCache: __TableCache<BidConfigS>) {
  ...
  removeOnInsert = (cb: (ctx: EventContext, row: BidConfigS) => void) => {
    return this.tableCache.removeOnInsert(cb);
  }

  onDelete = (cb: (ctx: EventContext, row: BidConfigS) => void) => {
